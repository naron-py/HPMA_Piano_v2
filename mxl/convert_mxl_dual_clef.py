...
def convert_mxl_with_dual_clef(mxl_file_path, output_path=None, custom_tempo=None):
    ...
    try:
        ...
        if custom_tempo is not None:
            ...
            print(f"📊 Musical Analysis (Custom Tempo):")
        else:
...
if __name__ == "__main__":
    ...
    try:
        if args.batch_convert:
            ...
            # Step 2: Parse file to get original tempo
            try:
                score = converter.parse(mxl_file_to_convert)
                metadata = extract_musical_metadata(score)
                original_tempo = metadata['tempo_bpm']
            except Exception as e:
                print(f"⚠️  Warning: Could not read original tempo from file. Using default 120 BPM.")
                original_tempo = 120
            
            # Step 3: Get custom tempo
            custom_tempo = get_custom_tempo(original_tempo)
            
            # Step 4: Convert with selected tempo
            print(f"\n🔄 Converting with tempo: {custom_tempo} BPM")
            success = convert_mxl_with_dual_clef(mxl_file_to_convert, args.output_file, custom_tempo)
            sys.exit(0 if success else 1)
            ...
            # Get original tempo and custom tempo
            try:
                score = converter.parse(mxl_file_to_convert)
                metadata = extract_musical_metadata(score)
                original_tempo = metadata['tempo_bpm']
